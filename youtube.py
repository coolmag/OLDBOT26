...
class YouTubeDownloader:
    """
    🎵 Aurora Downloader Engine (v7.2 - MediaConnect Bypass).
    ...
    async def _download_with_yt_dlp(self, url_or_query: str, target_path: Path, source_name: str) -> DownloadResult:
        ...
        # 👇 ИСПРАВЛЕНИЕ: Убираем 'tv' (DRM) и добавляем 'mediaconnect' (не требует JS Solver!)
        if cookie_file and "YouTube" in source_name:
            # web - с куками, mediaconnect - обходит JS и блокировки
            youtube_args['player_client'] = ['web', 'mediaconnect', 'web_embedded']
            if is_valid_token:
            ...
            # Если кук нет, пробуем мобильные и mediaconnect
            youtube_args['player_client'] = ['mediaconnect', 'ios', 'android_vr', 'web_embedded']
